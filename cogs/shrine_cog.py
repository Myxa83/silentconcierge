...
FOOTER_TEXT = "Silent Concierge | В пошуках страждущих"
...
class GSModal(discord.ui.Modal, title="Оновлення Гір Скору"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        embed.set_footer(text=FOOTER_TEXT, icon_url=self.cog.bot.user.display_avatar.url)
...
class PollResponseView(discord.ui.View):
    ...
    @discord.ui.button(label="Мій GS", style=discord.ButtonStyle.primary, emoji="⚔️", custom_id="per:gs")
    ...
    @discord.ui.button(label="Графік", style=discord.ButtonStyle.secondary, emoji="⏳", custom_id="per:sched")
    async def set_schedule(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(DetailsModal("Мій графік", "Вкажіть час", "Напр: 19:00-22:00", "schedule", self.cog))

    @discord.ui.button(label="Пропуск", style=discord.ButtonStyle.danger, emoji="✖️", custom_id="per:skip")
    ...
    @discord.ui.button(label="Відпустка", style=discord.ButtonStyle.gray, emoji="🌴", custom_id="per:vac")
    async def set_vacation(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(DetailsModal("Відпустка", "Період", "15.02 - 20.02", "vacation", self.cog))

    @discord.ui.button(label="Боси", style=discord.ButtonStyle.gray, emoji="🛑", custom_id="per:boss")
    async def refuse(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(BossModal(self.cog))

# --- В'Ю КЕРУВАННЯ РЕЙДОМ ---

class ShrinePartyView(discord.ui.View):
    def __init__(self, leader_id, boss_name, count, ts, cog):
        super().__init__(timeout=None)
        self.leader_id, self.boss_name, self.count, self.ts, self.cog = leader_id, boss_name, count, ts, cog
    ...
    async def update_embed(self, interaction):
        ...
        for m in self.members:
            icon = "👑" if m == self.leader_id else "⚔️"
            gs = gs_data.get(str(m), "??")
            left = max(0, 5 - weekly.get(str(m), 0))
            m_list.append(f"{icon} <@{m}> [GS: **{gs}** | Зал: **{left}**]")
...
class ShrineCog(commands.Cog):
    ...
    async def cog_load(self):
        self.bot.add_view(PollResponseView(self))
        # Реєструємо ShrinePartyView як динамічний за custom_id не вдасться без збереження стану, 
        # тому рейди працюють до перезавантаження або потребують БД.
    ...
    async def scheduler(self):
        ...
        if now in ["09:00", "15:00"]:
            await self.run_dm_polling()
        if now == "15:15":
            await self.send_report_to_channel(self.report_channel_id)
    ...
    async def run_dm_polling(self, target=None):
        ...
        for m in members:
            if m.bot or str(m.id) in vacations or weekly.get(str(m.id), 0) >= 5:
                continue
            try:
                left = 5 - weekly.get(str(m.id), 0)
                gs = gs_data.get(str(m.id), "??")
                embed = discord.Embed(title="Вітаю! Нагадування Black Shrine", 
                                    description=f"Ваш GS: **{gs}**\nЗалишилось: **{left}** босів\n\nКоли Вам буде комфортно пройти босів сьогодні?", color=0x2ecc71)
    ...
    async def send_report_to_channel(self, channel_id):
        ...
        for m in role.members:
            ...
            status = "✅" if done >= 5 else "⏳"
            lines.append(f"{status} **{m.display_name}** | GS: `{gs_data.get(uid, '??')}` | Боси: `{done}/5` | Графік: *{sched_data.get(uid, 'Не вказано')}*")

        embed = discord.Embed(title="📊 Список Black Shrine на сьогодні", description="\n".join(lines) if lines else "Список порожній", color=0x3498db)
        embed.set_footer(text=FOOTER_TEXT, icon_url=self.bot.user.display_avatar.url)
    ...
    @app_commands.choices(boss=[
        ...
        app_commands.Choice(name="Blue-clad Youth (Хлопчик в трусиках)", value="Blue-clad Youth"),
        ...
        app_commands.Choice(name="The Deposed Crown Prince (Принц)", value="Deposed Crown Prince")
    ])
    async def shrine_create(self, interaction: discord.Interaction, boss: app_commands.Choice[str], count: int, time_hhmm: int):
        try:
            ...
            return await interaction.response.send_message("❌ Формат часу: 1900", ephemeral=True)
        
        gs = self.load_json(GS_PATH).get(str(interaction.user.id), "??")
        left = max(0, 5 - self.load_json(WEEKLY_PATH).get(str(interaction.user.id), 0))
        
        embed = discord.Embed(title=f"⚔️ Black Shrine: {boss.name}", color=0x2ecc71)
        embed.description = f"Лідер: <@{interaction.user.id}>\nБосів: **{count}**\nЧас: <t:{ts}:t> (<t:{ts}:R>)"
        embed.add_field(name="Учасники (1/5)", value=f"👑 <@{interaction.user.id}> [GS: **{gs}** | Зал: **{left}**]", inline=False)
        embed.set_footer(text=FOOTER_TEXT, icon_url=self.bot.user.display_avatar.url)
        
        view = ShrinePartyView(interaction.user.id, boss.name, count, ts, self)
        await interaction.response.send_message(embed=embed, view=view)
        
        msg = await interaction.original_response()
        try: await msg.create_thread(name=f"Рейд {boss.name}")
        except: pass
