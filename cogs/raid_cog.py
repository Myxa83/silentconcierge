...
def build_embed(bot, user_id):
    ...
    embed.add_field(name="\u200b", value=f"**Шепотіть:** {raid_data['nick']}\n\n", inline=False)
    embed.add_field(name="\u200b", value=f"**Найм:** <t:{найм_timestamp}:t>\n\n", inline=False)
    embed.add_field(name="\u200b", value=f"**Сервер: Камасільва 5** *(уточнити в ПМ)*\n\n", inline=False)
    embed.add_field(name="\u200b", value=f"**Старт:** <t:{start_timestamp}:t>, після босів LoML\n\n", inline=False)
    embed.add_field(name="\u200b", value="**Шлях:** Хан → Бруд → Феррід → CTG на Футурума *(між босами 3–4 хв)*\n\n", inline=False)
    embed.add_field(name="\u200b", value="**Боси:** 3 рівня\n\n", inline=False)
    embed.add_field(name="\u200b", value="**Примітка:** Якщо ви забукіровали місце в альянсі, не протискайте прийняти до відведеного часу.\n\n", inline=False)
    embed.add_field(name="\u200b", value=f"**Слотів:** {raid_data['taken']} ✅ **Залишилось:** {залишилось}\n\n", inline=False)
