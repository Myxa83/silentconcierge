...
SAFE_WORDS = {
    "бляшка", "бляшки", "бляшку", "бляшкою", "бляшці", "бляшках",
    "бляшками", "бляшковий", "бляшкова", "бляшкове", "бляшкові",
    "бляшкового", "бляшкової", "бляшкових",
    "стріляти", "стріляє", "стріляють", "стріляв", "стріляла",
    "стріляли", "стрілятися", "пристріляти", "застріляти",
    "відстріляти", "постріляти",
    "мудрий", "мудра", "мудре", "мудрі", "мудрого", "мудрої",
    "мудрим", "мудрість", "мудрості",
    "сукня", "сукні", "сукню", "сукнею", "сукнях", "сукнями",
    "сукупність", "сукупності", "сукупний", "сукупна", "сукупне",
    "сукупні", "сукупного", "сукупної",
    "лохнесс", "трахбах", "трахтарах", "трахтрах", "єбрр",
}

SAFE_WORD_PREFIXES = (
    "бляшк", "стріля", "пристріля", "застріля", "відстріля", "постріля",
    "мудр", "сукн", "сукуп", "херсон", "херув", "херес", "херсонес",
    "бляшан", "блявк", "блягузк", "блякл", "блямб", "бляск", "блях",
    "хуейськ", "хербст", "херд", "хере", "херей", "херлі", "херман",
    "херст", "еббот", "еббі", "ебегнейл", "ебен", "ебер", "ебол",
    "ебоніт", "ебро", "ебулі", "ебігейл", "ебіґейл", "йоббік",
    "дерма", "дермат", "дермаль", "курвіметр", "сучев", "трахк",
    "трахкот", "турябистр", "турєбистр", "турїбистр", "лохин", "лохвиц",
    "лохмат",
)

EXACT_SWEAR_WORDS = {
    "asshole", "bastard", "bullshit", "cunt", "dickhead", "motherfucker",
    "slut", "whore", "виродок", "гнида", "дебіл", "дебілка", "довбень",
    "довбенька", "ідіот", "ідіотка", "лох", "лошара", "мерзота", "мразь",
    "падло", "паскуда", "покидьок", "придурок", "придурка", "сволота",
    "тварюка", "ублюдок", "чмо",
}
...
# Будь-які розділювачі, включно з пробілами.
# Використовуємо ТІЛЬКИ для повних слів/послідовностей з 3-4+ букв,
# щоб не склеювати два нормальні сусідні слова.
SEP_ANY = r"[\s\W_]*"

# Символи без пробілів. Потрібно для коротких коренів типу єб/еб,
# щоб "є бажаючі", "є баг", "є баланс" НЕ перетворювалися на мат.
SEP_SYMBOL = r"[^\w\s]*"
...
SWEAR_PATTERNS = [
    ...
    rf"(?<![{LETTER}])(?:h|x){SEP_ANY}u{SEP_ANY}(?:y|i)",

    # пизд / пізд
    rf"(?<![{LETTER}])п[иіiы][зz3][дd][{LETTER}]*",
    rf"(?<![{LETTER}])п{SEP_ANY}[иіiы]{SEP_ANY}[зz3]{SEP_ANY}[дd]",
    rf"(?<![{LETTER}])pizd[a-zа-яіїєґё]*",
    rf"(?<![{LETTER}])p{SEP_ANY}i{SEP_ANY}z{SEP_ANY}d",

    # Повне "ебать" транслітом, включно з e bat / e-bat / y e b a t.
    # Ставимо ДО короткого eb, щоб ebat цензурувалося цілком, а не лише "eb".
    rf"(?<![{LETTER}])(?:y{SEP_ANY}|j{SEP_ANY})?e{SEP_ANY}b{SEP_ANY}a{SEP_ANY}t(?:[a-zа-яіїєґё]*)?(?![{LETTER}])",
    rf"(?<![{LETTER}])(?:yo|jo){SEP_ANY}b(?:{SEP_ANY}a{SEP_ANY}t)?[a-zа-яіїєґё]*",

    # єб / еб / їб / йоб - короткий корінь, тому БЕЗ пробілів
    rf"(?<![{LETTER}])(?:єб|еб|їб|иб|йоб)[{LETTER}]*",
    rf"(?<![{LETTER}])(?:[еєe]{SEP_SYMBOL}[бb]|ї{SEP_SYMBOL}[бb]|й{SEP_SYMBOL}о{SEP_SYMBOL}[бb])",

    # заєб / заїб / зайоб / заеб
    rf"(?<![{LETTER}])за(?:єб|еб|їб|йоб)[{LETTER}]*",
    rf"(?<![{LETTER}])z(?:a|а)(?:eb|yob)[a-zа-яіїєґё]*",

    # сука / суки / суку / сукою / суками
    ...
    rf"(?<![{LETTER}])f{SEP_ANY}u{SEP_ANY}c{SEP_ANY}k",
    rf"(?<![{LETTER}])fck[a-zа-яіїєґё]*",
    rf"(?<![{LETTER}])shit[a-zа-яіїєґё]*",
    rf"(?<![{LETTER}])s{SEP_ANY}h{SEP_ANY}i{SEP_ANY}t",
    ...
    # повторені літери та розділювачі
    rf"(?<![{LETTER}])б+{SEP_SYMBOL}л+{SEP_SYMBOL}[я@]+(?:{SEP_SYMBOL}(?:д|т|ц|ь)+[{LETTER}]*)?(?![{LETTER}])",
    rf"(?<![{LETTER}])[хx]+{SEP_ANY}[уy]+{SEP_ANY}[йяєїиеюi]+",
    rf"(?<![{LETTER}])п+{SEP_ANY}[иіiы]+{SEP_ANY}[зz3]+{SEP_ANY}[дd]+",
    # коротке єб/еб - знову БЕЗ пробілів
    rf"(?<![{LETTER}])(?:[еєe]+{SEP_SYMBOL}[бb]+|ї+{SEP_SYMBOL}[бb]+|й+{SEP_SYMBOL}о+{SEP_SYMBOL}[бb]+)",
    rf"(?<![{LETTER}])f+{SEP_ANY}u+{SEP_ANY}c+{SEP_ANY}k+",
    rf"(?<![{LETTER}])s+{SEP_ANY}h+{SEP_ANY}i+{SEP_ANY}t+",
]
...
def load_filter_dictionary_from_mongo() -> None:
    ...
    if safe_prefixes:
        SAFE_WORD_PREFIXES = tuple(
            sorted(set(SAFE_WORD_PREFIXES).union(safe_prefixes), key=len, reverse=True)
        )
...
def clean_message_text(text: str) -> tuple[str, int]:
    """
    Чистить звичайний текст.
    URL тимчасово захищаємо, щоб не ловити мат у посиланнях.
    """
    total_count = 0
...
async def collect_attachment_files(
    ...
    for attachment in message.attachments:
        try:
            ...
            if LOG_TO_CONSOLE:
                print(
                    f"[MATY_OFF] Не вдалося прочитати вкладення "
...
class MatyOffCog(commands.Cog):
    ...
    def add_swear_count(self, member: discord.Member, count: int):
        ...
        self.data["months"][month].setdefault(
            user_id,
            {"count": 0, "name": member.display_name},
        )
    ...
    async def on_message(self, message: discord.Message):
        ...
        if LOG_TO_CONSOLE:
            print(
                ...
                f"user={message.author} msg_id={message.id} "
                f"count={swear_count} original={message.content!r} "
        ...
        if DELETE_ORIGINAL_MESSAGE:
            try:
                ...
                if LOG_TO_CONSOLE:
                    print(f"[MATY_OFF] Помилка видалення повідомлення: {error}")
        ...
        try:
            ...
            if LOG_TO_CONSOLE:
                print(f"[MATY_OFF] Помилка відправки з файлами: {error}")
            try:
                ...
                if LOG_TO_CONSOLE:
                    print(f"[MATY_OFF] Помилка відправки без файлів: {second_error}")
    ...
    async def give_monthly_award(self, month: str):
        ...
        if channel is None:
            try:
                ...
                if LOG_TO_CONSOLE:
                    print(f"[MATY_OFF] Не вдалося знайти award channel: {error}")
                return
            ...
            if role:
                ...
                if previous_winner_id:
                    ...
                    if previous_member and role in previous_member.roles:
                        try:
                            ...
                            if LOG_TO_CONSOLE:
                                print(f"[MATY_OFF] Не вдалося зняти роль: {error}")
                ...
                if winner_member:
                    try:
                        ...
                        if LOG_TO_CONSOLE:
                            print(f"[MATY_OFF] Не вдалося видати роль: {error}")
        ...
        embed = discord.Embed(
            ...
            description=(
                ...
                "Муха все бачила.\n"
                "Муха все записала.\n"
                "Муха розчаровано поставила печатку."
            ),
        ...
        try:
            ...
            if LOG_TO_CONSOLE:
                print(f"[MATY_OFF] Помилка відправки нагороди: {error}")
            return
